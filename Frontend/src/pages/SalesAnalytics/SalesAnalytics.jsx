...
const SalesAnalytics = () => {
    ...
    if (loading) return (
        <div className="analytics-wrapper">
            <div className="analytics-container text-center text-light py-5">
    ...
    if (!data) return (
        <div className="analytics-wrapper">
            <div className="analytics-container text-light text-center py-5">Failed to load data.</div>
        </div>
    );
            ...
            <div className="analytics-container">
                ...
                <div className="stats-row">
                    {[
                        ...
                        <div className="summary-card" key={i} style={{ borderLeft: `4px solid ${s.color}` }}>
                ...
                <div className="charts-row">
                    <div className="chart-card">
                        <h5 className="chart-title"><i className="bi bi-pie-chart me-2"></i>Payment Methods</h5>
                        {pieData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={260}>
                                <PieChart>
                                    ...
                                    <Tooltip formatter={(v, n, p) => [`${v} orders (₹${p.payload.amount?.toFixed(2)})`, n]} />
                                </PieChart>
                            </ResponsiveContainer>
                    ...
                    <div className="chart-card">
                        <h5 className="chart-title"><i className="bi bi-bar-chart me-2"></i>7-Day Sales Trend</h5>
                        {barData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={260}>
                ...
                <div className="user-sales-card">
                    <h5 className="chart-title"><i className="bi bi-people me-2"></i>Sales by User</h5>
                    <div className="table-responsive">
                        <table className="table table-dark table-hover align-middle">
                            <thead>
                                <tr>
                                    <th>User</th><th>Email</th><th>Today's Sales</th>
                                    <th>Today's Orders</th><th>Total Revenue</th><th>Total Orders</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.userSalesSummary?.map(u => (
                                    <tr key={u.userId}>
                                        ...
                                        <td className="text-warning">₹{u.todaySales?.toFixed(2) || "0.00"}</td>
                                        <td>{u.todayOrders || 0}</td>
                                        <td className="text-success fw-bold">₹{u.totalRevenue?.toFixed(2) || "0.00"}</td>
                                        <td>{u.totalOrders || 0}</td>
                                    </tr>
                                ))}
                                {(!data.userSalesSummary || data.userSalesSummary.length === 0) && (
                                    <tr><td colSpan={6} className="text-center text-muted py-3">No user data available</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
